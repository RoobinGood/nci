...
Scm.prototype._isAncestor = function(rev1, rev2, callback) {
	this._run({
		cmd: 'git',
		args: [
...
Scm.prototype.getChanges = function(rev1, rev2, callback) {
	...
	Steppy(
		...
		function(err, currentRev) {
			...
			if (rev1) {
				self._isAncestor(rev1, rev2, this.slot());
			} else {
				this.pass(null);
			}
		},
		...
		function(err, currentRev, stdout) {
			...
			_(changes).each(function(commit) {
				self._isAncestor(rev1, commit.id, group.slot());
			});
		},
		function(err, currentRev, changes, isAncestorResults) {
			changes = _(changes).filter(function(commit, index) {
				return isAncestorResults[index];
			});
