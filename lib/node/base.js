...
Node.prototype.getExecutorWaitReason = function(project, params) {
	...
	console.log('checkForScmChangesOnly', params.checkForScmChangesOnly)
	console.log('maxExecutorsCount', this.maxExecutorsCount)
	console.log('project.executorSlotCount', project.executorSlotCount)
	console.log('getFreeExecutorsCount', this.getFreeExecutorsCount())
	...
	if (!params.checkForScmChangesOnly) {
		if (this.getFreeExecutorsCount() < project.executorSlotCount) {
	...
	if (this.maxExecutorsCount < project.executorSlotCount) {
		waitReason = this.name +
			': project cannot reserve more slots than executor have';
	}
...
Node.prototype.getFreeExecutorsCount = function() {
	var busySlotCount = _(this.executors).reduce(function(acc, executor) {
		return acc + executor.project.executorSlotCount || 1;
	}, 0);

	return this.maxExecutorsCount - busySlotCount;
};
